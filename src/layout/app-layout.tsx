...
import { getProducers } from 'apiSdk/producers';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['producer', 'delivery-person'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ProducerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/producers` : null),
    () => getProducers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Producer',
      path: isTenantUser ? `/producers/view/${data?.[0]?.id}` : '/producers',
      entity: 'producer',
      service: AccessServiceEnum.PROJECT,
      icon: FiFilm,
    },
    { name: 'Cart', path: '/carts', entity: 'cart', service: AccessServiceEnum.PROJECT, icon: FiShoppingBag },
